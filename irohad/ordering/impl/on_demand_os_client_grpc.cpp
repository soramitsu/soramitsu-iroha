  ...
  void handleProposalResponse(
      ...
      std::optional<
          std::pair<std::shared_ptr<shared_model::interface::Proposal const>,
                    iroha::ordering::BloomFilter256>> const &ref_proposal) {
    if (!response.has_proposal_hash()) {
      log->info("Remote node has no proposal.");
    ...
    if (auto proposal_result = proposal_factory->build(response.proposal());
    ...
    if (ref_proposal.has_value()) {
      std::shared_ptr<shared_model::interface::Proposal const> local_proposal =
          ref_proposal.value().first;

      iroha::getSubscription()->notify(
          iroha::EventTypes::kRemoteProposalDiff,
          iroha::ordering::RemoteProposalDownloadedEvent{
              local_proposal,
              remote_proposal,
      ...
      iroha::getSubscription()->notify(
          iroha::EventTypes::kOnProposalResponse,
          iroha::ordering::ProposalEvent{std::move(remote_proposal), round});
    else {
      ...
      [round,
       ...
       proposal_factory(utils::make_weak(proposal_factory_))] {
        ...
        handleProposalResponse(
            maybe_log, maybe_proposal_factory, response, round, ref_proposal);
      });
}
